...
class subscriberUtils {
    static async notifySubscriberOnSubscription(subscriberEmail: string): Promise<void> {
        try {
            const message = 'You have subscribed to NdevuSpace.com successfully! Thank you for subscribing.'; 
    ...
    static async notifyAllSubscribersAboutUpdates(blogTitle: string, message: string): Promise<void> {
        try {
            const subscribers = await SubscriberService.getAllSubscribers(); // Fetch all subscribers
